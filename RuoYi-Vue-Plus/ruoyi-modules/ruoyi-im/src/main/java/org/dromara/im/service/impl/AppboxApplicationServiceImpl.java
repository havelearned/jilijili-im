...
public class AppboxApplicationServiceImpl implements IAppboxApplicationService {
     ...
     */
    @Override
    public AppboxApplicationVo queryById(Long id) {
    ...
    private void validEntityAfterSave(AppboxApplication add) {
        if (add.getChannelStatus() == 1) {
            ...
            application.setPv(0L);
            application.setUv(0L);
    ...
    @Override
    public List<AppboxApplicationVo> getTreeInfo(Long id) {
    ...
    @Override
    public Boolean addUserClass(Long appId) {
        AppUser appUser = new AppUser();
        appUser.setUserId(Long.valueOf(getCurrentUserIdToString()));
        appUser.setAppId(appId);
        return appUserMapper.insert(appUser) > 0;
    }

    @Override
    public Boolean removeUserClass(Long appId) {
        validRepeatedAddAppBox(appId);
     ...
     * 校验用户是否重复加入应用
     */
    public void validRepeatedAddAppBox(Long appId) {
        String userId = getCurrentUserIdToString();
        LambdaQueryWrapper<AppUser> lqw = Wrappers.lambdaQuery();
        lqw.eq(AppUser::getUserId, userId)
            .eq(AppUser::getAppId, appId);
